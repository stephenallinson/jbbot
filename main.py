...
bros = {
    "Stephen": characters.STEPHEN,
    "Jason": characters.JB,
    "Connor": characters.CONNOR,
    "Blair": characters.BLAIR,
}

bro, character = random.choice(list(bros.items()))
...
if __name__ == "__main__":
    ...
    FIRST_SYSTEM_MESSAGE = character

    FIRST_USER_MESSAGE = {
        "role": "user",
        "content": "Hi there, ready for another Sunday Funday gaming session?",
    }
    openai_manager.chat_history.append(FIRST_SYSTEM_MESSAGE)
    openai_manager.chat_history.append(FIRST_USER_MESSAGE)

    while True:
        new_prompt = input(f"\nYou are now chatting with: {bro}\n\nNext question? \n\n")
